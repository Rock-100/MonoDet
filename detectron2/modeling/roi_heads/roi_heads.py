...
class ROIHeads(torch.nn.Module):
    ...
    def label_and_sample_proposals(
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            ##########################################
            ignore_box_index = (targets_per_image.gt_classes == -1).nonzero()
            if ignore_box_index.shape[0]:
                ignore_anchor_index = matched_idxs == ignore_box_index[0]
                for i in range(1, ignore_box_index.shape[0]):
                    ignore_anchor_index = ignore_anchor_index | (matched_idxs == ignore_box_index[i])
                ignore_anchor_index = ignore_anchor_index & (matched_labels == 1)
                matched_labels[ignore_anchor_index] = -1
            ##########################################
